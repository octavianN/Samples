...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron"
  ...
  <sch:pattern>
    ...
    <sch:rule context="table">
      ...
      <sch:assert test="$minColumsNo >= $reqColumsNo" sqf:fix="addCells" subject="title">Cells are missing. (The
    ...
    <sch:rule context="li">
      ...
      <sch:report test="ends-with((.//text()[normalize-space()!=''])[last()], ';')" sqf:fix="removeSemicolon removeSemicolonList"
    ...
    <sch:rule context="text()[matches(., '(http|www)\S+')][local-name(parent::node()) != 'xref']">
      <sch:report test="true()" role="info" sqf:fix="convertToLink">
      ...
      <sqf:fix id="convertToLink">
        ...
        <sqf:user-entry name="name">
          <sqf:description><sqf:title>Specify the link text:</sqf:title></sqf:description>
        </sqf:user-entry>
        <sqf:stringReplace regex="(http|www)\S+">
          <xref href="$0" format="html" scope="external"><xsl:value-of select="$name"/></xref>
        </sqf:stringReplace>
      </sqf:fix>
    </sch:rule>
  </sch:pattern>
